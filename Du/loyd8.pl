...
nkroku(X, X, B, 0, _, B) :- !.
nkroku(X,Y, S, N, A, B):- N>0, tah(X, T, NT), 
                                    \+member(NT, A),
                                    N1 is N - 1, 
...
%posunDolu(+X,-T, -Y) posune 0 nahoru v listu, kde T je temp promenna
...
%map(+X, -Y) kde X je cislo hry a Y je jeho pozice na mrizce (kvuli jednoduchosti manhattansky normy)
map(1,[1,1]).
map(2,[1,2]).
map(3,[1,3]).
map(4,[2,1]).
