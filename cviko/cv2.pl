...
otoc([X|Xs],Y):-otoc(Xs,Z),pridejk(X,Z,Y). % kdyz chces otocit [H|T] otoc T
                                           % a pripoj za to 
%rev(+S1, -S2) :- S2 je S1 pozpatku
rev(X,Y) :- rev(X, [], Y).
rev([], A, A).
rev([X|Xs], A, R) :- rev(Xs, [X|A], R).


%pripoj(X,Y, Z) pripoji seznam Y za seznam X
pripoj([], X , X).
pripoj([X|Xs], Y, Z) :- pripoj(Xs, Y, Z1), Z = [X|Z1]. %mozne zapsat jako  pripoj([X|Xs], Y, [X|Z1]) :- pripoj(Xs, Y, Z1).
